			...
			<div class="col-md-12">
				<div style="color:#00aced;">
					<?php if($posts[0]['purpose'] !== "Rent") {?>
						...
						</label> '.$posts[0]['phase_name'].' <label style="color:#00aced;"> » 
							...
							<strong>
								<span style="color:#00aced; line-height: 50px;">
									<?php if($posts[0]['purpose'] !== "Rent") {?>
										...
										For <?= $posts[0]['purpose'] ?> in  
										<?= $posts[0]['society_name'].' '.$posts[0]['phase_name'] ?>
									<?php } else {?>
										<?= count($posts)?> Results 
										For <?= $posts[0]['purpose'] ?> in  
										<?= $posts[0]['society_name'].' '.$posts[0]['phase_name'] ?>
				...
				<div class="detail-product1" style="background: #f5f5f5;border-radius: 7px;    border: solid 1px #ebebeb;">
					<div class="heading">
						<?php if($posts[0]['purpose'] !== "Rent") {?>
							...
							<?= $posts[0]['purpose'] ?> in  <?= $posts[0]['society_name'] ?>
							<?= $posts[0]['phase_name'] ?>
						<?php } else {?>
							Locations For 
							<?= $posts[0]['purpose'] ?> in  <?= $posts[0]['society_name'] ?>
					...
					<div class="contain-wrapp padding-top20 padding-bot20">	
						<div class="row">
							<div class="cat_box" style="padding-left: 10px">
								<ul class="smaps_list">		
									<?php 
										$this->db->where('phase_id',$posts[0]['phase_id']);
										$this->db->order_by('sector_name','asc');
										$res = $this->db->get('sector')->result_array();
									
									foreach($res as $row) { ?>
										<?php if($posts[0]['purpose'] != "Rent"){?>
											<?php if($posts[0]['property_type'] == "Plots"){?>
												<li class="l"><a href="<?= base_url('home/sector_posts_plot/'.$row['sector_id'])?>"><?= $row['sector_name'] ?></a>
													(<?php $this->db->where('sector',$row['sector_id']);
											...
											<?php } else if($posts[0]['property_type'] == "Homes"){?>
												<li class="l"><a href="<?= base_url('home/sector_posts_home/'.$row['sector_id'])?>"><?= $row['sector_name'] ?></a>
													(<?php $this->db->where('sector',$row['sector_id']);
											...
											<?php } else if($posts[0]['property_type'] == "Commercial"){?>
												<li class="l"><a href="<?= base_url('home/sector_posts_commercial/'.$row['sector_id'])?>"><?= $row['sector_name'] ?></a>
													(<?php $this->db->where('sector',$row['sector_id']);
											...
											<?php } ?>
										<?php } else { ?>
											<li class="l"><a href="<?= base_url('home/sector_posts_rent/'.$row['sector_id'])?>"><?= $row['sector_name'] ?></a>
												(<?php $this->db->where('sector',$row['sector_id']);
