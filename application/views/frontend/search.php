							...
							<strong>
								<span style="color:#00aced; line-height: 50px;">
									<?php if($posts[0]['purpose'] !== "Rent") {?>
										...
										For <?= $posts[0]['purpose'] ?> in  <?= $posts[0]['city_name'] ?>
									<?php } else {?>
										<?= count($posts)?> Results 
										For <?= $posts[0]['purpose'] ?> in  <?= $posts[0]['city_name'] ?>
									<?php } ?>
		...
		<div class="container">
			<div class="row">
				<div class="detail-product1" style="background: #f5f5f5;border-radius: 7px;    border: solid 1px #ebebeb;">
					<div class="heading">
						<?php if($posts[0]['purpose'] !== "Rent") {?>
							Locations of <?= $posts[0]['property_type'] ?> For 
							<?= $posts[0]['purpose'] ?> in  <?= $posts[0]['city_name'] ?>
						<?php } else {?>
							Locations For 
							<?= $posts[0]['purpose'] ?> in  <?= $posts[0]['city_name'] ?>
						<?php } ?>
					...
					<div class="contain-wrapp padding-top20 padding-bot20">	
						<div class="row">
							<div class="cat_box" style="padding-left: 10px">
								<ul class="smaps_list">		
									<?php 
										$this->db->where('city_id',$posts[0]['city_id']);
										$this->db->order_by('society_name','asc');
										$res = $this->db->get('society')->result_array();
									
									foreach($res as $row) { ?>
										<?php if($posts[0]['purpose'] != "Rent"){?>
											<?php if($posts[0]['property_type'] == "Plots"){?>
												<li class="l"><a href="<?= base_url('home/society_posts_plot/'.$row['society_id'])?>"><?= $row['society_name'] ?></a> 
													(<?php $this->db->where('society',$row['society_id']);
											...
											<?php } else if($posts[0]['property_type'] == "Homes"){?>
												<li class="l"><a href="<?= base_url('home/society_posts_home/'.$row['society_id'])?>"><?= $row['society_name'] ?></a>
													(<?php $this->db->where('society',$row['society_id']);
											...
											<?php } else if($posts[0]['property_type'] == "Commercial"){?>
												<li class="l"><a href="<?= base_url('home/society_posts_commercial/'.$row['society_id'])?>"><?= $row['society_name'] ?></a>
													(<?php $this->db->where('society',$row['society_id']);
											...
											<?php } ?>
										<?php } else { ?>
											<li class="l"><a href="<?= base_url('home/society_posts_rent/'.$row['society_id'])?>"><?= $row['society_name'] ?></a>
												(<?php $this->db->where('society',$row['society_id']);
							...
							<div class="bottom-details">
								<div class="col-md-7">
									...
									<div class="col-md-12" id="details">
										...
										<img style="height:150px; width:150px;" src="<?= base_url('profile_images/'.$row['profile_image'])?>" class="alignright" alt="">
